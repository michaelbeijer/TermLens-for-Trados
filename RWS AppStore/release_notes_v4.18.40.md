...
### Added
- **SuperMemory Quick Add (Ctrl+Alt+M)** — capture terms and corrections from the Trados editor into your SuperMemory knowledge base vault. Also available via right-click in the editor grid. The dialog pre-fills from your source/target selection and adapts the correction label to your target language (e.g. "Correct Dutch form"). Optionally appends the term to the active translation prompt's terminology table so the correction takes effect on the next Ctrl+T
