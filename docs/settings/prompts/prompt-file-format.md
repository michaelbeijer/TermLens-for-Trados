...
The plugin automatically prepends a system prompt to every AI call. This system prompt includes language pair information, termbase terms (based on your [AI Context settings](../ai-settings.md)), and TM matches when enabled. The content you write in a `.svprompt` file is the **user prompt** — it is sent after the system prompt.
