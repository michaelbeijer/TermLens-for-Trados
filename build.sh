#!/bin/bash
# Build, package, and deploy TermLens for Trados Studio.
# Trados Studio must be CLOSED before running this script.
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$SCRIPT_DIR/src/TermLens"
DIST_DIR="$SCRIPT_DIR/dist"
BUILD_DIR="$PROJECT_DIR/bin/Release"
DOTNET="${HOME}/.dotnet/dotnet"

PACKAGES_DIR="$LOCALAPPDATA/Trados/Trados Studio/18/Plugins/Packages"
UNPACKED_DIR="$LOCALAPPDATA/Trados/Trados Studio/18/Plugins/Unpacked/TermLens"

echo "=== Building TermLens ==="
"$DOTNET" build "$PROJECT_DIR/TermLens.csproj" -c Release

echo ""
echo "=== Packaging TermLens.sdlplugin (OPC format) ==="
mkdir -p "$DIST_DIR"
rm -f "$DIST_DIR/TermLens.sdlplugin"
python "$SCRIPT_DIR/package_plugin.py" "$BUILD_DIR" "$DIST_DIR/TermLens.sdlplugin"
...
if [ -d "$UNPACKED_DIR" ]; then
    echo "  Removing stale Unpacked/TermLens..."
    rm -rf "$UNPACKED_DIR"
...
cp "$DIST_DIR/TermLens.sdlplugin" "$PACKAGES_DIR/TermLens.sdlplugin"
echo "  Installed: $PACKAGES_DIR/TermLens.sdlplugin"
